...
public class Part : MonoBehaviour
{
    ...
    public IEnumerator Assemble()
    {
        ...
        do
        {
                ...
                for (int i = 0; i < core.m_StickAvailableSeat.Count; ++i)
                {
                    if (core.m_StickAvailableSeat[i].Equals(curGridIdx)) //드래그 중 붙을 수 있는 지역안에 들어옴
                    {
                        ...
                        if (iIdx + 1 == iTargetIdx)
                        {
                            ...
                            {
                                if (m_bNeedToStickHead)
                                    transform.localPosition = new Vector3(transform.localPosition.x + 0.025f, transform.localPosition.y);
                        ...
                        else if (iIdx - grid.m_iXcount == iTargetIdx)
                        {
                            ...
                            {
                                if (m_bNeedToStickHead)
                                    transform.localPosition = new Vector3(transform.localPosition.x, transform.localPosition.y + 0.025f);
            ...
            if (bFollowCursor && Input.GetMouseButtonUp(0))//클릭 뗏을때
            {
                ...
                if (m_partType.Equals(PART_TYPE.BODY))
                ...
                for (int i = 0; i < core.m_StickAvailableSeat.Count; ++i)
                {
                    ...
                    if (core.m_StickAvailableSeat[i].Equals(curGridIdx)) // Stick!!!!!
                    {
                        ...
                        GetComponent<SpriteSheet>().CheckAround(false);
                        GetComponent<SpriteRenderer>().sortingLayerName = "Monster";
                        //						GetComponent<ParticleSystemRenderer>().sortingLayerName = "Object_Particle";
                        ...
                        if (m_bAttackAvailable)
                        {
                            ...
                            if (m_weaponType != WEAPON_TYPE.BOW)
                                ...
                                transform.GetChild(0).GetChild(0).GetComponent<DPSpritePalette>().PaletteIndex = m_iPaletteSkin;
    ...
    void DestroyPart_WhenPathBreaked()
    {
        ...
        if (m_objCurParentPart.GetComponent<Part>().m_bDestroied || m_objCurParentPart.transform.parent.name.Equals("Field"))
        {//Breaked Path -> Disabled
    ...
    public void PartDestroyed()
    {
        ...
        {
            GetComponent<SpriteParticleEmitter.DynamicEmitter>().enabled = false;
            GameObject.Find("Player").BroadcastMessage("DestroyPart_WhenPathBreaked", SendMessageOptions.DontRequireReceiver);
            return;
        }
