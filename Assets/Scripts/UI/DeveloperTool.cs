...
public class DeveloperTool : MonoBehaviour
{
    ...
    enum DEBUG_STATE { CREATE_CARAVAN, CREATE_RAIDER, CREATE_HERO, CREATE_HERO_GARRISON, END };
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                switch (curDebug)
                {
                    ...
                    case DEBUG_STATE.CREATE_HERO:
                        ObjectFactory.getInstance.Create_Party(GridMgr.getInstance.GetGridIdx(Camera.main.ScreenToWorldPoint(Input.mousePosition)), PARTY_TYPE.RAID, System.Convert.ToInt32(GameObject.Find("Dev_CreateHeroNum").GetComponent<UILabel>().text));
                        break;

                    case DEBUG_STATE.CREATE_HERO_GARRISON:
                        ObjectFactory.getInstance.Create_Party(GridMgr.getInstance.GetGridIdx(Camera.main.ScreenToWorldPoint(Input.mousePosition)), PARTY_TYPE.GARRISON, System.Convert.ToInt32(GameObject.Find("Dev_CreateHeroGarrisonNum").GetComponent<UILabel>().text));
                        break;
                }
    ...
    {
        Debug_Escape();
        strCurDebugLabel = "Create Hero\n" + (HERO)(System.Convert.ToInt32(GameObject.Find("Dev_CreateHeroNum").GetComponent<UILabel>().text));
        MouseDebugOn(true, DEBUG_STATE.CREATE_HERO);
    }

    void Debug_CreateHeroGarrison()
    {
        Debug_Escape();
