...
Shader "Sprites/Sprite Color FX/Sprite Color Outline Gradient Horizontal Mask"
{
	...
	SubShader
	{
		...
		void surf(Input IN, inout SurfaceOutput o)
		{
			...
			fixed4 maskPixel = tex2D(_MaskTex, IN.uv_MainTex) * IN.color;
			fixed4 spriteColor = tex2D(_MainTex, IN.uv_MainTex) * IN.color;

			fixed4 mainColor = spriteColor;
			if (maskPixel.a < (1.0 - _AlphaThreshold))
				mainColor = lerp(outlinePixel.a * _OutlineColor, outlinePixel.a * _OutlineColor2, IN.localPos.x * _GradientScale + _GradientOffset);
