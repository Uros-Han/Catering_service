...
fixed4 DPSampleSpriteTexture(float2 uv, float index)
{
	...
	#if !(SHADER_API_MOBILE  || SHADER_TARGET_GLSL || SHADER_API_D3D9)
	[unroll(32)]
	#endif
...
fixed4 DPBilinear(float2 uv, float index)
{
	...
	fixed4 tl = DPSampleSpriteTexture(uv, index);
	fixed4 tr = DPSampleSpriteTexture(uv + float2(1.0f/ _SpriteSize.z, 0), index);
	fixed4 bl = DPSampleSpriteTexture(uv + float2(0, 1.0f/ _SpriteSize.w), index);
	fixed4 br = DPSampleSpriteTexture(uv + float2(1.0f/ _SpriteSize.z, 1.0f/ _SpriteSize.w), index);
