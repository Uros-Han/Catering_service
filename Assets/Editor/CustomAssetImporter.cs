...
internal sealed class CustomAssetImporter : AssetPostprocessor
{
    ...
    public void OnPostprocessTexture(Texture2D texture)
    {
        ...
        string fileName = assetPath.Substring(assetPath.LastIndexOf('/') + 1, assetPath.LastIndexOf('.') - assetPath.LastIndexOf('/') - 1);

        if (fileName.Contains("arms") && texture.height == 32)
        {
            ...
            textureImporter.spritePivot = new Vector2(0.5f, 0.25f);
            Debug.Log("Pivot : " + textureImporter.spritePivot);
        ...
        for (int r = 0; r < rowCount; ++r)
        {
            for (int c = 0; c < colCount; ++c)
            {
                ...
                meta.rect = new Rect(c * spriteSize, (rowCount - r - 1) * spriteSize, spriteSize, spriteSize);
                meta.name = fileName + "_" + (r * colCount + c);
                metas.Add(meta);
            }
        }
